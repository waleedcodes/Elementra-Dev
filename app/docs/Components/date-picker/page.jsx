...
import { Copy, CopyIcon, Calendar as CalendarIcon } from "lucide-react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs";
import { motion, AnimatePresence } from "framer-motion";

const DatePickerDocPage = () => {
  const [date, setDate] = useState(null);

  const codeExample = `import { DatePicker } from "@/components/ui/date-picker";
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8 sm:mb-12">
        <h1 className="text-3xl sm:text-4xl font-bold mb-3 sm:mb-4 text-foreground">Date Picker</h1>
        <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">
          Date picker component combining popover date selector & formatted date display for forms.
        </p>
      </div>
      ...
      <div className="space-y-8 sm:space-y-12">
        ...
        <section className="space-y-4">
          ...
          <div className="relative">
            <div className="bg-card border border-border rounded-lg p-4 font-mono text-sm overflow-x-auto">
              <code className="text-foreground">npx elementra-ui add date-picker</code>
            </div>
          </div>
        </section>
              ...
              <AnimatePresence mode="wait">
                <TabsContent value="preview">
                  <div className="rounded-lg border border-border bg-background p-6 flex flex-col items-center justify-center min-h-[220px]">
                    <div className="flex h-10 w-64 items-center gap-2 rounded-md border border-border bg-card px-3 text-sm text-muted-foreground shadow-xs cursor-pointer">
...
export default DatePickerDocPage;
