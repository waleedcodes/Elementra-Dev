...
import { Copy, CopyIcon, FileText } from "lucide-react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs";
import { motion, AnimatePresence } from "framer-motion";

const TextareaDocPage = () => {
  const [text, setText] = useState("");

  const codeExample = `import { Textarea } from "@/components/ui/textarea";
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8 sm:mb-12">
        <h1 className="text-3xl sm:text-4xl font-bold mb-3 sm:mb-4 text-foreground">Textarea</h1>
        <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">
          A multi-line text input field supporting resizing, validation states, and configurable rows.
        </p>
      </div>
                ...
                <TabsContent value="preview">
                  <div className="rounded-lg border border-border bg-background p-6 flex flex-col items-center justify-center min-h-[220px]">
                    <textarea
                      ...
                      className="w-full max-w-md p-3 rounded-md border border-input bg-background text-foreground text-sm outline-none focus:ring-2 focus:ring-ring"
                    />
                  </div>
                </TabsContent>
